...
PROGRAM EVALUATION
  ...
  if(scalene) compute_triangle=Ini_Read_Logical('Compute_triangle',.False.)
  ...
  if(equilateral) then 
     ...
     OPEN(UNIT=901,FILE=trim(OUTPUT_DIR)//"/PS.txt",FORM='FORMATTED',STATUS='REPLACE')
     OPEN(UNIT=902,FILE=trim(OUTPUT_DIR)//"/"//GFILENAME,FORM='FORMATTED',STATUS='REPLACE')
  end if
  
#ifndef MPI  
  OPEN(UNIT=903,FILE=trim(OUTPUT_DIR)//"/F_NL-RE.txt",FORM='FORMATTED',STATUS='REPLACE')
  ISTART=1
  IEND=KPOINTS
  INTERVAL=1
#endif
#ifdef MPI
  OPEN(UNIT=903+MY_ID,FILE=trim(OUTPUT_DIR)//"/"//FNLFILENAME,FORM='FORMATTED',STATUS='REPLACE')
  ISTART=MY_ID+1
